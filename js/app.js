...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <p class="category">Category: <span class="product_rating"> ${product.category}</span></p>
      <h2 class="price">Price: $ ${product.price}</h2>
      <p class="product_rating">Avg Rating:  <span class="price">${product.rating.rate}</span> | Total Rating: <span class="price">${product.rating.count} </span></p>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn search-button">add to cart</button>
